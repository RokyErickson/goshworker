...
//Makes a new Proc
func newProc(startready, routinePool chan chan *Work, opts Opts) *proc {
	...
	cmd := Gosh(opts, Opts{In: r1, Out: w2, Err: w3}).Bake()

	return &proc{cmd: cmd, in: w1, out: r2, err: r3, RoutinePool: routinePool, StartReady: startready}
}
